...
public class EcRenderTable extends Basis{
	...
	public boolean ini(){
		try {
			...
			if(!f.exists()) throw new Exception("Definition File["+definitionFileUri+"] not Found!!");
			...
			for(int i = 0;i < nd1.getLength();i++){
				...
				String id = nd1item.getAttributes().getNamedItem("id") != null ? nd1item.getAttributes().getNamedItem("id").getNodeValue() : null;
				String type = nd1item.getAttributes().getNamedItem("type") != null ? nd1item.getAttributes().getNamedItem("type").getNodeValue() : null;
				String alias = nd1item.getTextContent();
				
				if(id == null) throw new Exception("Definition File["+definitionFileUri+"] Setting Fail,Column ID can't be null!!");
			...
			try {
				if(ini()){
					for(Map<Object,String> colMp  : columnInfos){
						if(colMp.get(ColumnInfo.TYPE) == null) continue;
						if(compareValue(colMp.get(ColumnInfo.TYPE),  ColumnQueryType.TEXT.toString())){
							...
							if(queryValue != null) {
								if(queryCriterions == null) queryCriterions = new ArrayList<>();
